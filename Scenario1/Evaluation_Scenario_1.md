...
```@example scenario1
@parameters β=0.1 γ=0.2 ν=0.15
@variables t S(t) I(t) R(t) V(t)
D = Differential(t)
n_stratify = 3
Ns = map(v -> toparam(v), variables(:N, 1:n_stratify))
Ss = map(v -> v(t), variables(:S, 1:n_stratify, T = FnType))
Is = map(v -> v(t), variables(:I, 1:n_stratify, T = FnType))
Rs = map(v -> v(t), variables(:R, 1:n_stratify, T = FnType))
Vs = map(v -> v(t), variables(:V, 1:n_stratify, T = FnType))

using Random
Random.seed!(123)
defs = Dict(x => rand() for x in Iterators.flatten((Ns, Ss, Is, Rs, Vs)))

eqs = [map((s, N) -> D(s) ~ -β / N * I * s - ν * s, Ss, Ns);
       map((i, N) -> D(i) ~ β / N * i * S - γ * i, Is, Ns);
       map((r, i) -> D(r) ~ γ * i, Rs, Is);
       map((v, s) -> D(v) ~ ν * s, Vs, Ss);
       S ~ sum(Ss)
       I ~ sum(Is)
       R ~ sum(Rs)]
@named model = ODESystem(eqs; defaults = defs)
